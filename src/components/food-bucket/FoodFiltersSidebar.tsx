...
import { DIETARY_TAGS, DISEASE_TAGS, REGION_TAGS } from "@/lib/constants";

export default function FoodFiltersSidebar() {
  ...
  // Helper to format tag names
  const formatTag = (tag: string) => {
    if (tag === 'non_veg') return 'Non-Vegetarian';
          ...
          <div className="flex flex-col gap-3 max-h-48 overflow-y-auto pr-2">
            {DIETARY_TAGS.map((tag) => (
              <label key={tag} className="flex items-center gap-3 cursor-pointer group">
                <input 
                  type="checkbox" 
                  checked={currentDietary.includes(tag)}
                  onChange={() => handleToggle("dietary", tag)}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {DISEASE_TAGS.map((tag) => {
              const isActive = currentDisease.includes(tag);
              return (
                <button 
                  key={tag}
                  onClick={() => handleToggle("disease", tag)}
